FUNCTION zabs_fm_encerrar_florada.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  CHANGING
    ...
    IF sy-subrc EQ 0.
      IF lwa_florada_tela-atwrt EQ '1'.
        ...
        IF sy-subrc EQ 0.
          ...
          ENDIF.
        ENDIF.
      ELSEIF lwa_florada_tela-atwrt EQ '2'
          OR lwa_florada_tela-atwrt EQ '3'.
        REFRESH lr_atnam.
        INSERT INITIAL LINE INTO TABLE lr_atnam ASSIGNING FIELD-SYMBOL(<lwa_atnam>).
        IF sy-subrc EQ 0.
          <lwa_atnam> = 'IEQ'.
          <lwa_atnam>-low = 'CIT-FLORADA'.
        ENDIF.

        INSERT INITIAL LINE INTO TABLE lr_atnam ASSIGNING <lwa_atnam>.
        IF sy-subrc EQ 0.
          <lwa_atnam> = 'IEQ'.
          <lwa_atnam>-low = 'CIT-DATA-ENCERRA'.
        ENDIF.

        INSERT INITIAL LINE INTO TABLE lr_atnam ASSIGNING <lwa_atnam>.
        IF sy-subrc EQ 0.
          <lwa_atnam> = 'IEQ'.
          <lwa_atnam>-low = 'CIT-SAFRA'.
        ENDIF.

        SELECT atinn, adzhl, atnam,
               atfor, anzst, anzdz
          FROM cabn
          INTO TABLE @DATA(lt_cabn_flor)
         WHERE atnam IN @lr_atnam.

        REFRESH lr_atinn.
        lr_atinn = CORRESPONDING #( lt_cabn_flor MAPPING low = atinn ).
        lwa_atinn = 'IEQ'.
        ...
        SELECT h~mdocm, h~mdtyp, h~aslvl, h~tplnr_fl,
          ...
          INTO TABLE @DATA(lt_glmdhdr_flor)
          FROM /agri/glmdhdr AS h
          INNER JOIN /agri/glmdatv AS v
          ON v~mdocm EQ h~mdocm
          INNER JOIN cabn AS c
          ON c~atinn EQ v~atinn
         WHERE h~mdtyp    EQ 'ZPTA' "Medições ao nível de safra
           AND h~tplnr_fl EQ @cs_mdhdr-tplnr_fl
           AND h~mpgrp    EQ 'ENCERRAR_FLORADA'
           AND h~kfrst    EQ @/agri/cl_fmpr_process_ticket=>mc_kfrst
           AND h~canceled EQ @abap_false
           AND v~atinn    IN @lr_atinn[].

        SORT lt_glmdhdr_flor BY mdocm DESCENDING.

        READ TABLE ct_mdatv INTO lwa_safra_tela
            WITH KEY atinn = lwa_atributo-safra.
        IF sy-subrc EQ 0.
          DATA(lt_doc_safra) = lt_glmdhdr_flor[].
          DELETE lt_doc_safra WHERE atinn NE lwa_atributo-safra.

          LOOP AT lt_doc_safra INTO DATA(lwa_doc_safra).
            IF lwa_doc_safra-atinn NE lwa_atributo-safra.
              CONTINUE.
            ELSE.
              IF lwa_doc_safra-atwrt NE lwa_safra_tela-atwrt.
                DELETE lt_glmdhdr_flor WHERE mdocm = lwa_doc_safra-mdocm.
              ENDIF.
            ENDIF.
          ENDLOOP.
        ENDIF.
...
*-- Data de Lançamento
        IF lwa_florada_tela-atwrt EQ '2'.
          READ TABLE lt_glmdhdr_flor INTO DATA(lwa_glmdhdr_flor)
            WITH KEY atnam = 'CIT-FLORADA'
                     atwrt = lv_florada_1.
        ELSEIF lwa_florada_tela-atwrt EQ '3'.
          IF lwa_terrain-x-flhdr-ownshp EQ lc_propriedade-proprio.
            READ TABLE lt_fl_doc INTO lwa_terrain INDEX 1.
            IF sy-subrc EQ 0.
              DATA(lt_faz_estimativa) = lt_glmdhdr_join[].
              DELETE lt_faz_estimativa WHERE mpgrp NE 'FAZ-ESTIMATIVA'.
*-- Verifica florada do doc. medição do grupo ENCERRAR_FLORADA
              READ TABLE ct_mdatv INTO DATA(lwa_mdatv)
                WITH KEY atinn = lwa_atributo-florada.
              IF sy-subrc EQ 0.
*-- Verifica se existe doc. medição para grupo FAZ-ESTIMATIVA da florada
                READ TABLE lt_faz_estimativa INTO DATA(lwa_faz_estimativa)
                  WITH KEY atinn = lwa_atributo-florada
                           atwrt = lv_florada_2.
                IF sy-subrc EQ 0.
                  READ TABLE lt_glmdhdr_flor INTO lwa_glmdhdr_flor
                    WITH KEY atnam = 'CIT-FLORADA'
                             atwrt = lv_florada_2.
                ELSE.
                  READ TABLE lt_glmdhdr_flor INTO lwa_glmdhdr_flor
        ...
        IF lwa_glmdhdr_flor-mdocm IS NOT INITIAL.
          READ TABLE lt_glmdhdr_flor INTO DATA(lwa_data_encerra)
            WITH KEY mdocm = lwa_glmdhdr_flor-mdocm
                     atnam = 'CIT-DATA-ENCERRA'.
          IF sy-subrc EQ 0.
            CLEAR lv_data_char.
            ...
            IF sy-subrc EQ 0.
              ...
              lv_data_temp = lwa_data_encerra-atflv.
              WRITE lv_data_temp TO lv_data_encerra.
              ADD 1 TO lv_data_encerra.
              IF lv_data_encerra IS NOT INITIAL.
                INSERT INITIAL LINE INTO TABLE lr_budat ASSIGNING <lwa_budat>.
                IF sy-subrc EQ 0.
                  <lwa_budat> = 'IBT'.
                  <lwa_budat>-low = lv_data_encerra.
                  <lwa_budat>-high = lv_data_char.
                ENDIF.
              ENDIF.
            ENDIF.
          ENDIF.
...
*-- Tipo de Movimento
  ...
  IF sy-subrc EQ 0.
*-- Posting Date
    IF <lwa_safra> IS ASSIGNED.
      REFRESH lr_budat.
      INSERT INITIAL LINE INTO TABLE lr_budat ASSIGNING <lwa_budat>.
      IF sy-subrc EQ 0.
        <lwa_budat> = 'IBT'.
        IF lv_data_encerra IS NOT INITIAL
        AND lv_data_encerra LT <lwa_safra>-fim_safra.
          <lwa_budat>-low = lv_data_encerra.
        ELSE.
          <lwa_budat>-low = <lwa_safra>-inicio_safra.
        ENDIF.
        <lwa_budat>-high = <lwa_safra>-fim_safra.
        IF lwa_terrain-x-flhdr-ownshp EQ lc_propriedade-proprio.
          lt_faz_estimativa[] = lt_glmdhdr_join[].
          DELETE lt_faz_estimativa WHERE mpgrp NE 'FAZ-ESTIMATIVA'.
*-- Verifica florada do doc. medição do grupo ENCERRAR_FLORADA
          READ TABLE ct_mdatv INTO lwa_mdatv
            WITH KEY atinn = lwa_atributo-florada.
          IF sy-subrc EQ 0.
*-- Verifica se existe doc. medição para grupo FAZ-ESTIMATIVA da florada
            READ TABLE lt_faz_estimativa INTO lwa_faz_estimativa
              WITH KEY atinn = lwa_atributo-florada
                       atwrt = lwa_mdatv-atwrt.
            IF sy-subrc NE 0.
              CLEAR lv_menge_x.
            ENDIF.
          ENDIF.

          DELETE lt_glmdhdr_join WHERE mpgrp     NE 'ENCERRAR_FLORADA'
                                    OR atinn_out NE 'CIT-DATA-ENCERRA'.
          SORT lt_glmdhdr_join BY mdocm     DESCENDING
                                  mpgrp     ASCENDING
                                  atinn_out ASCENDING.
          lv_data_inicio = <lwa_budat>-low.
          lv_data_fim    = <lwa_budat>-high.

          LOOP AT lt_glmdhdr_join INTO DATA(ls_glmdhr_join).
            CLEAR lv_data_char.
            CALL FUNCTION 'CEVA_CONVERT_FLOAT_TO_CHAR'
              EXPORTING
                float_imp  = ls_glmdhr_join-atflv
                format_imp = lv_i
              IMPORTING
                char_exp   = lv_data_char.
            IF lv_data_char IS NOT INITIAL.
              lv_data_inicio = lv_data_char.
              IF lv_data_inicio LE lv_data_fim.
                <lwa_budat>-low = lv_data_char.
                EXIT.
              ENDIF.
            ENDIF.
          ENDLOOP.
...
*-- Material
    REFRESH lr_matnr.
    INSERT INITIAL LINE INTO TABLE lr_matnr
      ASSIGNING FIELD-SYMBOL(<lwa_matnr>).
    IF sy-subrc EQ 0.
      <lwa_matnr> = 'IBT'.
      <lwa_matnr>-low = '1'.
    ...
    CASE lwa_terrain-x-flhdr-ownshp.
      WHEN lc_propriedade-proprio.
        REFRESH lr_bwart.
        SELECT bwart AS low
          INTO CORRESPONDING FIELDS OF TABLE @lr_bwart
          FROM t156
        ...
        IF cs_mdhdr-mpgrp EQ 'ENCERRAR_FLORADA'.
          INSERT INITIAL LINE INTO TABLE lr_mpgrp
            ASSIGNING <lwa_mpgrp>.
          IF sy-subrc EQ 0.
            <lwa_mpgrp> = 'IEQ'.
          ...
          SORT lt_glmdhdr_flor BY mdate DESCENDING
                                  mtime DESCENDING.

          READ TABLE lt_glmdhdr_flor  INTO lwa_glmdhdr_flor INDEX 1.
          IF sy-subrc EQ 0.
            lv_data_temp = lwa_glmdhdr_flor-atflv.
            WRITE lv_data_temp TO lv_data_encerra.
            IF lv_data_encerra IS NOT INITIAL.
              READ TABLE lr_budat ASSIGNING <lwa_budat> INDEX 1.
              IF sy-subrc EQ 0.
                IF <lwa_budat>-low LT lv_data_encerra.
                  <lwa_budat>-low = lv_data_encerra.
                ENDIF.
              ENDIF.
            ENDIF.
...
*-- Branch
        INSERT INITIAL LINE INTO TABLE lr_werks
...
*-- Próprio
  ...
  LOOP AT lt_ausp INTO DATA(lwa_caracteristica).
    ...
    IF lwa_caracteristica-atinn EQ lwa_imovel-atinn.
      IF lwa_caracteristica-atwrt NE lv_cod_imovel.
        lv_delete = abap_true.
      ENDIF.
    ELSEIF lwa_caracteristica-atinn NE lwa_talhao-atinn.
      IF lwa_caracteristica-atwrt EQ lv_talhao.
        lv_delete = abap_true.
      ENDIF.
    ENDIF.
...
*-- Próprio
  ...
  IF <lwa_producao_real> IS ASSIGNED.
    ...
    EXPORT lv_menge_char TO MEMORY ID 'ZVTX_PRODU_REAL'.
  ENDIF.
  ...
  LOOP AT ct_mdatv INTO lwa_mdatv.
