REPORT zabs_fmacm_upload.

INCLUDE zabs_inc_fmacm_upload_top.
INCLUDE zabs_inc_fmacm_upload_f01.
...
START-OF-SELECTION.
  ...
  IF lines( t_sheets_out ) > 0.
    IF p_bdo IS NOT INITIAL. "Create Accomplishment Sheet
      PERFORM accomplishment_sheet_create USING t_sheets_out.
    ELSEIF p_upd IS NOT INITIAL. "Update Accomplishment Sheet
      PERFORM accomplishment_sheet_update USING t_sheets_out.
