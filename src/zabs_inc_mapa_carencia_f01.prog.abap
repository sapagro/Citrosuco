...
FORM initialize_global_data .
  ...
  REFRESH: gt_farm, gt_glflot, gt_mara, gt_fmfpitm, gt_fmfpcom,
           gt_fmocindx, gt_fmocopr, gt_fmoccom, gt_matdoc,
           gt_output.
...
FORM get_terrains .
  ...
  DATA: lt_mat_class TYPE TABLE OF bapi1003_alloc_list,
        ...
        lt_carencia  TYPE STANDARD TABLE OF ly_carencia INITIAL SIZE 0,
...
*        READ TABLE lt_carencia INTO DATA(ls_carencia)
...
*              WITH KEY charact = 'FAZ_DIAS_CARENCIA'.
*            IF sy-subrc EQ 0.
*              IF ls_val_num-value_from IS NOT INITIAL.
*                lv_carencia = ls_val_num-value_from.
*                INSERT INITIAL LINE INTO TABLE lt_carencia
*                  ASSIGNING FIELD-SYMBOL(<ls_carencia>).
*                IF sy-subrc EQ 0.
*                  <ls_carencia>-matnr = ls_fmocindx-matnr.
*                  <ls_carencia>-carencia = lv_carencia.
*                ENDIF.
*              ENDIF.
*            ENDIF.
*          ENDIF.
*        ELSE.
*          lv_carencia = ls_carencia-carencia.
*        ENDIF.
*      ENDIF.
      CLEAR lv_carencia.
      ...
      IF sy-subrc EQ 0.
        ...
        <ls_output>-dias_carencia  = lv_carencia.
        <ls_output>-data_aplicacao = ls_fmocindx-budat.
        IF lv_carencia IS NOT INITIAL.
          <ls_output>-data_bloqueio = ls_fmocindx-budat + lv_carencia.
        ENDIF.
...
FORM check_restriction_period CHANGING lrt_budat TYPE gy_budat_range.
  ...
  DATA: lt_constants TYPE zabs_tty_vkey_const,
        ...
        lv_k1val     TYPE zabs_del_k1val VALUE 'CARENCIA',
  ...
  IF sy-subrc EQ 0.
    IF ls_constant-cnval2 CO ' 0123456789'.
      ...
      lv_carencia = ls_constant-cnval2.
    ENDIF.
  ENDIF.
  ...
  IF sy-subrc EQ 0.
    ...
    IF lv_carencia IS NOT INITIAL.
      <lrs_budat>-low = <lrs_budat>-low - lv_carencia.
    ENDIF.
  ENDIF.
...
FORM controls_display.
  ...
  PERFORM (lv_routine) IN PROGRAM zabs_rep_mapa_carencia IF FOUND.
