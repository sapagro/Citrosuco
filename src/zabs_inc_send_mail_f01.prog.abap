...
*-- Revisar fluxos de aprovação p/ grupo &1 na transação ZABS_VTMAIN !
    MESSAGE i334(zfmfp) WITH p_mpgrp.
...
*---Fetching Approvers
    SELECT *
      FROM zabst_apv_regiao
      INTO TABLE gt_apv_regiao
     WHERE beber1 IN s_region[].
    IF sy-subrc NE 0.
      MESSAGE i189(zabs_msgcls).
      LEAVE LIST-PROCESSING.
    ELSE.
      SORT gt_apv_regiao BY beber1.
    ENDIF.
...
*--Não existem dados para os parâmetros informados
  ...
  LOOP AT gt_mdhdr ASSIGNING FIELD-SYMBOL(<fs_mdhdr>).
    ...
    IF sy-subrc EQ 0.
      ...
      READ TABLE lt_glflot2 INTO DATA(ls_glflot2)
        WITH KEY tplnr_fl = <fs_mdhdr>-tplnr_fl BINARY SEARCH.
      IF sy-subrc EQ 0.
        READ TABLE lt_glflot1 INTO DATA(ls_glflot1)
          WITH KEY tplnr_fl = ls_glflot2-tplma BINARY SEARCH.
        IF sy-subrc EQ 0.
          READ TABLE gt_apv_regiao INTO DATA(ls_apv)
            WITH KEY beber1 = ls_glflot1-beber1 BINARY SEARCH.
          IF sy-subrc EQ 0.
            UNASSIGN <fs_value>.
            ASSIGN COMPONENT 'BEBER1' OF STRUCTURE <gfs_dyn_smdm> TO <fs_value>.
            IF <fs_value> IS ASSIGNED.
              <fs_value> = ls_apv-beber1.
            ENDIF.
          ENDIF.
        ENDIF.
      ENDIF.
...
*-- Detalhado
  ...
  LOOP AT <gfs_dyn_tmdm> ASSIGNING <gfs_dyn_smdm>.
    ...
    ELSE.
      ...
      ENDIF.
      IF lv_ok = abap_true.
        ASSIGN COMPONENT: 'EST_INICIAL'  OF STRUCTURE <gfs_saux> TO FIELD-SYMBOL(<fs_to4>),
                          'EST_ATUAL'    OF STRUCTURE <gfs_saux> TO FIELD-SYMBOL(<fs_to5>),
                          'EST_ANTERIOR' OF STRUCTURE <gfs_saux> TO FIELD-SYMBOL(<fs_to6>).
...
*--Field catalog prepare
  ...
  LOOP AT gt_fcat ASSIGNING FIELD-SYMBOL(<ls_fcat>).
    ...
    OR <ls_fcat>-fieldname = 'EST_INICIAL'
    OR <ls_fcat>-fieldname = 'EST_ANTERIOR'
    OR <ls_fcat>-fieldname = 'VAR_QTD_ANTERIOR'
    OR <ls_fcat>-fieldname = 'VAR_PERC_ANTERIOR'
    OR <ls_fcat>-fieldname = 'EST_ATUAL'
    OR <ls_fcat>-fieldname = 'VAR_QTD_ATUAL'
    OR <ls_fcat>-fieldname = 'VAR_PERC_ATUAL'
    OR <ls_fcat>-fieldname = 'EST_COLHEITA'
    OR <ls_fcat>-fieldname = 'MOT_EST'
    OR <ls_fcat>-fieldname = 'JUS_EST'
    OR <ls_fcat>-fieldname = 'MOT_ESTDESC'
    OR <ls_fcat>-fieldname = 'JUS_ESTDESC'.
...
*--Append Dynamic Columns to Field Catalog
  ...
  ls_fcat-fieldname  = 'BEBER1'.
  ls_fcat-col_pos    = lv_col_pos + 1.
  ls_fcat-fieldname  = 'BEBER1'.
  ls_fcat-scrtext_s  = ls_fcat-scrtext_m  = ls_fcat-scrtext_l =
  ls_fcat-reptext    = ls_fcat-tooltip = ls_fcat-coltext = 'Região'.
  APPEND ls_fcat TO gt_fcat.
  CLEAR ls_fcat.
...
FORM set_default_values .
  ...
  IF p_year IS INITIAL.
    ...
    IF sy-subrc EQ 0.
      READ TABLE lt_safras INTO DATA(ls_safra) INDEX 1.
      IF sy-subrc EQ 0.
...
* *&Get the Email id and User id Whom you want to Send  ******
  DATA:lt_receivers       TYPE STANDARD TABLE OF  somlreci1,
       ...
       ls_lt_message      LIKE LINE OF lt_message,
  ...
  SORT gt_apv_regiao BY beber1.
  LOOP AT <gfs_dyn_tmdm> ASSIGNING FIELD-SYMBOL(<fs_mdhdr>).
    ASSIGN COMPONENT 'BEBER1' OF STRUCTURE <gfs_dyn_smdm>
      TO FIELD-SYMBOL(<fs_value>).
    IF sy-subrc EQ 0.
      READ TABLE gt_apv_regiao TRANSPORTING NO FIELDS
        WITH KEY beber1 = <fs_value> BINARY SEARCH.
      IF sy-subrc EQ 0.
*        LOOP AT gt_apv_regiao INTO DATA(lv_apv) WHERE beber1 = <fs_value>.
        LOOP AT gt_apv_regiao INTO DATA(lv_apv) FROM sy-tabix.
          IF lv_apv-beber1 NE <fs_value>.
            EXIT.
          ENDIF.
          lv_bname = lv_apv-bname.
          ...
          FREE ls_lt_receivers.
          ls_lt_receivers-receiver   = lv_apv-bname.  "&----- Assign SAP User Id
  ...
  SORT lt_receivers.
  DELETE lt_receivers WHERE receiver IS INITIAL.
...
*&----------------------------------------------------------------------
*&-- Subject of the mail -------------&*
    psubject = 'Existem Imóveis pendentes de aprovação'.
    "&--  Body  of the mail ----------------&*
    CLEAR ls_lt_message.
    ls_lt_message-line = 'Prezado Aprovador,'.
    APPEND ls_lt_message TO lt_message.
*** insert Blank Line *********************************************
    CLEAR ls_lt_message.
    ls_lt_message-line = '                               '.
    APPEND ls_lt_message TO lt_message.
******* Assign your Text  below *************************************
    CLEAR ls_lt_message.
    ls_lt_message-line = 'Existe imóvel/ou imóveis pendente de liberação.'.
    APPEND ls_lt_message TO lt_message.
*** insert Blank Line{} *********************************************
    CLEAR ls_lt_message.
    ls_lt_message-line = '                                        '.
...
**********Assign your Text  below ********************************
    CLEAR ls_lt_message.
    ls_lt_message-line = 'Este e-mail foi gerado automaticamente, por favor não responda'.
    APPEND ls_lt_message TO lt_message.
*********************************************************************
**********& Send EMAIL MESSAGE  &*********************************
