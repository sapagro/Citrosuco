...
*  All Rights Reserved                                                 *
************************************************************************
* Report Name       :  ZABS_REP_BGACCOM_SUB                            *
...
*--Fetching Confirmation Data
  ...
  IF gv_crt IS NOT INITIAL
 AND gv_cnf IS NOT INITIAL.
...
*&---------------------------------------------------------------------*
*& Form BACKGRD_ACCOM_CREATE_CONFIRM
*&---------------------------------------------------------------------*
*& BACKGRD_ACCOM_CREATE_CONFIRM
*&---------------------------------------------------------------------*
FORM backgrd_accom_create_confirm.
 ...
 AND gv_cnf IS NOT INITIAL.
  ...
  IF lt_acbgp_cnf IS NOT INITIAL.
    LOOP AT lt_acbgp_cnf INTO DATA(ls_acbgp_cnf).
      ls_mackey_tmp-zzmackey = ls_acbgp_cnf-mackey.
      APPEND ls_mackey_tmp TO lt_mackey_tmp.
      CLEAR ls_mackey_tmp.
    ...
    SORT lt_mackey_tmp.
    DELETE ADJACENT DUPLICATES FROM lt_mackey_tmp COMPARING zzmackey.
  ENDIF.
...
*--Parallel Processing Logic
  IF p_par IS NOT INITIAL.
    PERFORM parallel_processing USING lt_mackey
                                      lt_mackey_tmp
                                      lt_acitm_crt
                                      lt_acbgp_cnf.
  ENDIF.
...
*  ENDIF.
  ...
  IF p_bgpro IS NOT INITIAL.
    CLEAR lv_flag.
  ENDIF.

  PERFORM accom_create USING lv_flag
                             lt_acitm_crt
                    CHANGING lt_achdr_crt
                             lt_acdoc.
...
*  ENDIF.
  ...
  PERFORM accom_conf USING lv_flag
                           lt_achdr_crt
                           lt_acbgp_cnf
                           lt_acdoc.
...
FORM fetch_buffer_accom_create USING pv_reprocess TYPE xfeld
  ...
  DATA : ls_acitm_crt TYPE zabs_bgp_accom.
...
*       AND posnr    IN s_pos
  ...
  DELETE ct_acitm_crt WHERE batch EQ 'B'.
  IF ct_acitm_crt IS INITIAL.
*      MESSAGE TEXT-015 TYPE zcl_abs_abap_maintain=>c_msgty_info
*                       DISPLAY LIKE zcl_abs_abap_maintain=>c_msgty_error.
*      leave LIST-PROCESSING.
    gv_crt = abap_true.
  ...
  IF p_par IS NOT INITIAL.
    ls_acitm_crt-batch = 'B'.
    ls_acitm_crt-aenam = sy-uname.
    ls_acitm_crt-aedat = sy-datum.
    ls_acitm_crt-aezet = sy-uzeit.
    MODIFY ct_acitm_crt FROM ls_acitm_crt TRANSPORTING batch aenam aedat aezet
     WHERE zzmackey NE space.
    MODIFY zabs_bgp_accom FROM TABLE ct_acitm_crt.
    COMMIT WORK AND WAIT.
  ENDIF.

*  SORT ct_acitm_crt BY zzmackey posnr.
...
FORM fetch_buffer_accom_confirm USING pv_reprocess TYPE xfeld
  ...
  DATA : ls_acbgp_cnf TYPE zabs_ac_bggrp.
...
*-- Fetching confirmations data
  ...
  DELETE ct_acbgp_cnf WHERE batch EQ 'B'.
  IF ct_acbgp_cnf IS INITIAL.
*    MESSAGE TEXT-016 TYPE zcl_abs_abap_maintain=>c_msgty_info
*                     DISPLAY LIKE zcl_abs_abap_maintain=>c_msgty_error.
*    LEAVE LIST-PROCESSING.
    gv_cnf = abap_true.
  ENDIF.
  ...
  IF p_par IS NOT INITIAL.
    ls_acbgp_cnf-batch = 'B'.
    ls_acbgp_cnf-aenam = sy-uname.
    ls_acbgp_cnf-aedat = sy-datum.
    ls_acbgp_cnf-aezet = sy-uzeit.
    MODIFY ct_acbgp_cnf FROM ls_acbgp_cnf TRANSPORTING batch aenam aedat aezet
     WHERE mackey NE space.
    MODIFY zabs_ac_bggrp FROM TABLE ct_acbgp_cnf.
    COMMIT WORK AND WAIT.
  ENDIF.
...
*            ls_error_dis-msgid = '00'.
*            ls_error_dis-msgno = '001'.
*            ls_error_dis-msgtyp = 'W'.
*            ls_error_dis-msgtxt = TEXT-014.
...
*              ls_error_dis-posnr = <fs_acitm_crt>-posnr.
            ...
            ENDIF. "<fs_acitm_crt>-duplicate
          ENDLOOP. "lt_acitm_crt
        ENDIF. "lt_acitm_crt
...
*          ls_ac_bggrp-aezet   = sy-uzeit.
          ...
          ELSE.
            ...
            IF sy-subrc EQ 0.
              "----------------------- Wave 3
              IF ls_messages-msgid EQ zcl_abs_abap_maintain=>c_msg_class_/agri/fmfp
             AND ls_messages-msgno EQ zcl_abs_abap_maintain=>c_msg_no_51.
                ...
                ls_error_log_cnf-msgid  = ls_messages-msgid.
                ls_error_log_cnf-msgno  = ls_messages-msgno.
                ls_error_log_cnf-msgtyp = zcl_abs_abap_maintain=>c_msgty_error.
                PERFORM message_build USING ls_messages
                                   CHANGING ls_error_log_cnf-msgtxt.
                ls_error_log_cnf-ernam = sy-uname.
                ls_error_log_cnf-erdat = sy-datum.
                ...
                ls_error_dis-msgid    = ls_messages-msgid.
                ls_error_dis-msgno    = ls_messages-msgno.
                ls_error_dis-msgtyp   = zcl_abs_abap_maintain=>c_msgty_error.
                ls_error_dis-msgtxt   = ls_error_log_cnf-msgtxt.
                APPEND ls_error_dis TO gt_str_accomlog.
              ELSE.
                "---------------------- Wave 3
...
*& PARALLEL_PROCESSING
*&---------------------------------------------------------------------*
FORM parallel_processing USING pt_mackey     TYPE tt_mackey
                               pt_mackey_tmp TYPE tt_mackey
                               pt_acitm_crt  TYPE zabs_tty_bgp_accom
                               pt_acbgp_cnf  TYPE zabs_tty_ac_baggrp.

  DATA: lt_mac_crt    TYPE tt_mackey,
        lt_mac_cnf    TYPE tt_mackey,
        lt_crt_mackey TYPE tt_mackey,
        lt_acitm_crt  TYPE zabs_tty_bgp_accom,
        lt_cnf_mackey TYPE tt_mackey,
        lt_acbgp_cnf  TYPE zabs_tty_ac_baggrp,
        ...
        lv_job        TYPE char10,
        lv_crt_cmp    TYPE flag,
        lv_flag       TYPE flag.

  lt_mac_crt[] = pt_mackey[].
  lt_mac_cnf[] = pt_mackey_tmp[].
...
*-- Schedule the jobs based on Participants data
  DO.
    ...
    REFRESH: lt_crt_mackey,lt_acitm_crt,lt_acbgp_cnf,lt_cnf_mackey.

*-- Collect the Participant to be used while creating Calculation run
    LOOP AT lt_mac_crt INTO DATA(ls_mac_crt) FROM 1 TO p_psize.
      APPEND ls_mac_crt TO lt_crt_mackey.
      READ TABLE pt_acitm_crt TRANSPORTING NO FIELDS
                              WITH KEY zzmackey = ls_mac_crt-zzmackey
                              BINARY SEARCH.
      IF sy-subrc EQ 0.
        DATA(lv_tabix) = sy-tabix.
        LOOP AT pt_acitm_crt INTO DATA(ls_acitm_crt) FROM lv_tabix.
          IF ls_acitm_crt-zzmackey <> ls_mac_crt-zzmackey.
            EXIT.
          ENDIF.
          APPEND ls_acitm_crt TO lt_acitm_crt.
        ENDLOOP."pt_acitm_crt
      ENDIF."pt_acitm_crt
    ENDLOOP."lt_mac_crt


    IF lt_mac_crt IS INITIAL.
      IF lv_crt_cmp IS NOT INITIAL.
        REFRESH gt_joblog.
      ENDIF.

      CLEAR: lv_crt_cmp.

      LOOP AT lt_mac_cnf INTO DATA(ls_mac_cnf) FROM 1 TO p_psize.
        APPEND ls_mac_cnf TO lt_cnf_mackey.

        READ TABLE pt_acbgp_cnf TRANSPORTING NO FIELDS
                                WITH KEY mackey = ls_mac_cnf-zzmackey
                                BINARY SEARCH.
        IF sy-subrc NE 0.
          CONTINUE.
        ENDIF.
        lv_tabix = sy-tabix.
        LOOP AT pt_acbgp_cnf INTO DATA(ls_acbgp_cnf) FROM lv_tabix .
          IF ls_acbgp_cnf-mackey <> ls_mac_cnf-zzmackey.
            EXIT.
          ENDIF.
          APPEND ls_acbgp_cnf TO lt_acbgp_cnf.
          CLEAR ls_acbgp_cnf.
        ENDLOOP. "pt_acbgp_cnf
      ENDLOOP. "lt_mac_cnf
    ENDIF. "lt_mac_crt
...
*-- Prepare Job name
    ...
    CONCATENATE lv_job_name lv_job '-' lv_char_index INTO lv_job_name.
    CONDENSE lv_char_index NO-GAPS.
...
*--Submit call
      IF lt_mac_crt IS NOT INITIAL.
        SUBMIT zabs_rep_bgaccom
          WITH p_mac   EQ lt_crt_mackey
          WITH p_acitm EQ lt_acitm_crt
          WITH p_par   EQ space
          WITH p_pro   EQ space
          WITH p_bgpro EQ lv_flag
          WITH p_rpro  EQ abap_true
          WITH p_acco  EQ abap_true
          WITH p_cnfm  EQ space
           VIA JOB lv_job_name
        NUMBER lv_job_number
           AND RETURN.
      ELSE.
        SUBMIT zabs_rep_bgaccom
          WITH p_mac   EQ lt_cnf_mackey
          WITH p_accnf EQ lt_acbgp_cnf
          WITH p_par   EQ space
          WITH p_pro   EQ space
          WITH p_bgpro EQ lv_flag
          WITH p_rpro  EQ abap_true
          WITH p_cnfm  EQ abap_true
          WITH p_acco  EQ space
           VIA JOB lv_job_name
        NUMBER lv_job_number
           AND RETURN.
      ENDIF.
...
*         targetgroup          = ls_parallel_pro_ctrl-srgrp
    ...
    IF lt_mac_crt IS INITIAL
   AND lt_mac_cnf IS NOT INITIAL.
      DELETE lt_mac_cnf FROM 1 TO p_psize.
    ENDIF.

    IF lt_mac_crt IS NOT INITIAL.
*-- Delete the processed records
      DELETE lt_mac_crt FROM 1 TO p_psize.

      IF lt_mac_crt IS INITIAL
     AND lt_mac_cnf IS NOT INITIAL.
        lv_crt_cmp = abap_true.
      ENDIF.
    ENDIF.

*-- Exit if all the Accomplishment and Confirmation data is scheduled in job
    IF lt_mac_crt IS INITIAL
   AND lt_mac_cnf IS INITIAL.
      EXIT.
    ENDIF.

    IF gv_active_sessions GE p_maxsn
    OR lv_crt_cmp IS NOT INITIAL.

      DATA(lv_maxsn) = p_maxsn.
      IF lv_crt_cmp IS NOT INITIAL.
        CLEAR lv_maxsn.
      ENDIF.
...
*-- greater than max number specified
      PERFORM sub_job_status_check USING lv_crt_cmp
                                         lv_maxsn.   " Max Jobs
  ...
  MESSAGE TEXT-013 TYPE zcl_abs_abap_maintain=>c_msgty_success.
...
FORM sub_job_status_check USING pv_crt_cmp TYPE flag
...
*-- Field Symbols
  ...
  DO.
    ...
    IF gv_active_sessions LT pv_maxjobs
      AND pv_crt_cmp IS INITIAL.
      EXIT.
    ELSEIF gv_active_sessions IS INITIAL.
      EXIT.
    ENDIF.
