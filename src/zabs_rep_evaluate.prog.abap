...
*-- Task list - operation/activity
    SELECT plnty, plnnr, plnkn, zaehl,
           vornr, steus, werks
      FROM plpo
      INTO TABLE @DATA(lt_plpo_actv)
      FOR ALL ENTRIES IN @lt_qals
     WHERE plnty EQ @lt_qals-plnty
       AND plnnr EQ @lt_qals-plnnr.
    "AND plnkn EQ @lt_qals-VORGLFNR.

    SORT lt_plpo_actv BY plnty plnnr plnkn.
...
*  ENDIF.
  ...
  LOOP AT lt_qasr ASSIGNING <ls_qasr>.
    ...
    IF sy-subrc NE 0.
      CONTINUE.
    ELSE.
      READ TABLE lt_fmfphdr INTO ls_fmfphdr
    ...
    IF sy-subrc NE 0
    AND so_ehdat[] IS NOT INITIAL.
      CONTINUE.
    ELSE.
      READ TABLE lt_plpo_actv INTO DATA(lwa_plpo)
        WITH KEY plnty = lwa_qals-plnty
                 plnnr = lwa_qals-plnnr
                 plnkn = lwa_qapp-vorglfnr BINARY SEARCH.
      IF sy-subrc NE 0.
        CLEAR lwa_plpo.
      ENDIF.
    ENDIF.
    ...
    MOVE-CORRESPONDING lwa_plpo  TO lwa_actqlt.
    MOVE-CORRESPONDING <ls_qasr> TO lwa_actqlt.
    MOVE-CORRESPONDING <ls_qasr> TO lwa_insc.
    lwa_actqlt-pruefer      = lwa_qapp-pruefer.
*-- Data Criação Ponto de Controle
    lwa_actqlt-entstehdat   = lwa_qapp-entstehdat.
    lwa_actqlt-entstehzt    = lwa_qapp-entstehzt.
    lwa_insc-original_input = <ls_qasr>-original_input.
    SHIFT lwa_qapp-userc1 LEFT DELETING LEADING '0'.
    lwa_insc-userc1         = lwa_qapp-userc1.
    SHIFT lwa_insc-userc1 LEFT DELETING LEADING '0'.
    lwa_actqlt-tplnr_fl = ls_fmfphdr-tplnr_fl.
*-- BOC T_T.KONNO-02.04.21
    lwa_actqlt-vbewertung = lwa_qapp-vbewertung.
*-- EOC T_T.KONNO-02.04.21
  ...
  SORT: gt_practqlt BY prueflos ppsortkey vorglfnr merknr,
        gt_insc BY prueflos vorglfnr merknr probenr.
...
FORM ticket_convert TABLES   lt_qapp
  ...
  DATA: lt_prnum_tmp TYPE RANGE OF /agri/fmprnum,
...
FORM dynamic_catalog_fill.
  ...
  lv_lines = lines( gt_fieldcat_fl ).

  IF gt_practqlt[] IS NOT INITIAL.
    SELECT * FROM qamv
      INTO TABLE @DATA(lt_qamv)
      FOR ALL ENTRIES IN @gt_practqlt
     WHERE prueflos EQ @gt_practqlt-prueflos
       AND vorglfnr EQ @gt_practqlt-vorglfnr.

    SORT lt_qamv BY verwmerkm.
    DELETE ADJACENT DUPLICATES FROM lt_qamv COMPARING verwmerkm.

    LOOP AT lt_qamv INTO DATA(lwa_qamv).
      READ TABLE gt_fieldcat_fl INTO lwa_fcat
        WITH KEY fieldname = lwa_qamv-verwmerkm.
      IF sy-subrc <> 0.
        lwa_fcat-fieldname = lwa_qamv-verwmerkm.
        lwa_fcat-scrtext_l = lwa_qamv-kurztext.
  ...
  LOOP AT gt_practqlt ASSIGNING <ls_practqlt>.
    ...
    OR lv_vorglfnr <> <ls_practqlt>-vorglfnr.
      ...
      READ TABLE gt_insc INTO lwa_insc
        WITH KEY prueflos = <ls_practqlt>-prueflos
                 vorglfnr = <ls_practqlt>-vorglfnr
                 merknr   = <ls_practqlt>-merknr
                 probenr  = <ls_practqlt>-probenr BINARY SEARCH.
      IF sy-subrc NE 0.
        CONTINUE.
      ENDIF.
      ...
      IF lwa_insc-katalgart1 IS NOT INITIAL.
        ...
        <lf_value> = lv_descr.
      ELSE.
        IF lwa_insc-verwmerkm EQ 'F_LONGIT'
        OR lwa_insc-verwmerkm EQ 'F_LATITU'.
          <lf_value> = lwa_insc-original_input.
        ELSE.
