...
*-- Refresh Global data
  REFRESH: gt_productions, gt_events, gt_output, gt_fill_complex.
...
*--BOC-T_T.KONNO-07.15.21
        lrt_zzcdate      TYPE RANGE OF erdat,
*--EOC-T_T.KONNO-07.15.21
        ...
        lv_zzhdate       TYPE zabs_del_hdate,
*--BOC-T_T.KONNO-07.15.21
        lv_objid         TYPE zabs_del_objid VALUE 'DATA',
        lv_klval         TYPE zabs_del_k1val VALUE 'ZZCDATE',
        lv_constante     TYPE zabs_del_cnval,
        lv_dias          TYPE i,
*--EOC-T_T.KONNO-07.15.21
        lv_pabrj         TYPE pabrj,
        lv_pabrp         TYPE pabrp.
...
*--BOC-T_T.KONNO-07.15.21
  DATA(lv_yesterday_bool) = abap_false.
*--EOC-T_T.KONNO-07.15.21
...
*--BOC-T_T.KONNO-07.15.21
    IF lv_begda = lv_endda.
      ...
      IF lv_yesterday = lv_begda.
        lv_yesterday_bool = abap_true.
      ENDIF.
    ENDIF.
*--EOC-T_T.KONNO-07.15.21
  ENDIF.
...
*  ENDIF.
  IF lv_yesterday_bool = abap_true.
    INSERT INITIAL LINE INTO TABLE lrt_zzhdate
      ASSIGNING FIELD-SYMBOL(<lrs_zzhdate>).
    IF sy-subrc EQ 0.
      <lrs_zzhdate> = 'IBT'.
  ...
  ELSE.
    INSERT INITIAL LINE INTO TABLE lrt_zzhdate
      ASSIGNING <lrs_zzhdate>.
    IF sy-subrc EQ 0.
      <lrs_zzhdate> = 'IBT'.
      lv_data_ref = sy-datum - 5.
      <lrs_zzhdate>-low = lv_data_ref.
      <lrs_zzhdate>-high = sy-datum.
    ENDIF.
  ENDIF.
*--EOC-T_T.KONNO-07.15.21
...
*--BOC-T_T.KONNO-07.15.21
  ...
  IF lv_yesterday_bool = abap_false.
    CALL METHOD zcl_abs_get_variants=>get_constant_single
      EXPORTING
        iv_objid  = lv_objid "DATA x ZABS_PEHCM
        iv_k1val  = lv_klval "ZZCDATE
      IMPORTING
        ev_cnval1 = lv_constante. "10

    IF lv_constante CO ' 0123456789'.
      SHIFT lv_constante LEFT DELETING LEADING '0'.
      CONDENSE lv_constante NO-GAPS.
      lv_dias = lv_constante.
    ENDIF.

    IF lv_dias IS NOT INITIAL.
      READ TABLE lrt_zzcdate ASSIGNING FIELD-SYMBOL(<lrs_zzcdate>) INDEX 1.
      IF sy-subrc EQ 0.
        IF <lrs_zzcdate>-high IS INITIAL
        AND <lrs_zzcdate>-low IS NOT INITIAL.
          <lrs_zzcdate>-high = <lrs_zzcdate>-low.
        ENDIF.
        lv_data_ref = <lrs_zzcdate>-low - lv_dias.
        <lrs_zzcdate>-low = lv_data_ref.
        <lrs_zzcdate>-sign = 'I'.
        <lrs_zzcdate>-option = 'BT'.
      ENDIF.
    ENDIF.
  ENDIF.
*--EOC-T_T.KONNO-07.15.21
...
*-- Fetch the productions and events from accomplishment sheets
  SELECT a~accom, a~ernam, a~erdat, a~zzturma,
         ...
         b~fintim, b~idactvl, b~menge, zzjust,
...
*    ENDIF.
    ...
    IF sy-subrc EQ 0.
      SORT lt_t549q BY begda.

      LOOP AT lt_prod_events INTO DATA(ls_prod_event).
        DATA(lv_tabix_del) = sy-tabix.

        IF lv_zzhdate NE ls_prod_event-zzhdate.
          DATA(lv_t549q_found) = abap_false.
          LOOP AT lt_t549q INTO DATA(ls_t549q).
*--BOC-BOC-T_T.KONNO-07.15.21
*            IF ls_prod_event-zzhdate BETWEEN ls_t549q-begda
*                                         AND ls_t549q-endda.
*              lv_t549q_found = abap_true.
**-- Tolerância de 3 dias no final da validade
*              ADD 3 TO ls_t549q-endda.
*              EXIT.
*            ENDIF.
            IF lv_yesterday_bool EQ abap_false.
              IF sy-datum BETWEEN ls_t549q-begda
                              AND ls_t549q-endda.
                lv_t549q_found = abap_true.
*-- Tolerância de 3 dias no final da validade
                ADD 3 TO ls_t549q-endda.
...
*--EOC-BOC-T_T.KONNO-07.15.21
        ...
        lv_zzhdate = ls_prod_event-zzhdate.
      ENDLOOP.
    ENDIF.
...
*--EOC-T_T.KONNO-07.15.21
    ...
    LOOP AT gt_productions INTO DATA(ls_production).
      ...
      IF sy-subrc EQ 0.
*--BOC-T_T.KONNO-07.15.21
        IF ls_pa9900-complex IS INITIAL.
          APPEND ls_production TO gt_fill_complex.
        ENDIF.
...
*--BOC-BOC-T_T.KONNO-07.15.21
    ls_dados-modificado_por = sy-uname.
    ls_dados-modificado_em = sy-datum.
    ls_dados-nome_arquivo = <fs_prod_events>-accom.
*--EOC-BOC-T_T.KONNO-07.15.21
...
*                WITH KEY fazenda_farm = ls_terrains-form BINARY SEARCH.
    ...
    SELECT SINGLE form
      FROM @lt_terrains AS terrains
      WHERE tplnr_fl = @<fs_prod_events>-tplnr
      INTO @DATA(lv_form).

    SELECT SINGLE btrtl
      FROM @lt_hcm_form AS hcm_form
      WHERE fazenda_farm = @lv_form
      INTO @DATA(lv_btrtl).

    IF sy-subrc EQ 0.
      DATA(lv_faz_btrtl) = lv_btrtl.
    ENDIF.
