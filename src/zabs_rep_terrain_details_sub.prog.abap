...
*--Fetching Crop Seasons data
  SELECT tplnr_fl, contr, cmnum, varia, season,
         datab, datbi, astat, loevm
    FROM /agri/glflcma
...
*--Fetching Measurement Document Attribute Values data
  ...
  ELSE.
    ...
    IF rb_lep IS NOT INITIAL. "imprimir 103/104
...
*--Deleting records based on radio button selected
      ...
      DELETE lt_glmdhdr WHERE mdocm IN rt_glmdatv.
    ENDIF.
...
*--Filling text based on radio button selected
  ...
  SORT gt_farm_subtot BY region.

*--- Wave 3
  REFRESH: gt_farm_subtot_f[], gt_farm_subtot_t[].
  LOOP AT gt_farm_subtot INTO lwa_farm_subtot.
**    IF lwa_farm_subtot-region IS NOT INITIAL.
    lwa_farm_subtot-region = ''.
    lwa_farm_subtot-reg_txt = ''.
    lwa_farm_subtot-reason = ''.
    COLLECT lwa_farm_subtot INTO gt_farm_subtot_f.
**    ENDIF.
    CLEAR lwa_farm_subtot.
  ENDLOOP.

  LOOP AT gt_farm_subtot_f INTO lwa_farm_subtot.
    IF lwa_farm_subtot-region IS INITIAL
   AND lwa_farm_subtot-rsdesc NE 'TOTAL'.
      lwa_farm_subtot-region = ''.
      lwa_farm_subtot-reg_txt = ''.
      lwa_farm_subtot-reason = ''.
      lwa_farm_subtot-rsdesc = 'TOTAL'.
      COLLECT lwa_farm_subtot INTO gt_farm_subtot_t.
    ENDIF.
    CLEAR lwa_farm_subtot.
  ENDLOOP.
...
*      lwa_terrain_dtls-farm2  = lwa_farm_dtls-farm.
...
FORM initialize_global_data.
  ...
  REFRESH: gt_terrain_dtls, gt_farm_subtot, gt_farm_subtot_f,
           gt_att_content_hex.
  CLEAR: gv_print.
...
*--Calling generated Function Module
  CALL FUNCTION lv_fm_name "'/1BCDWB/SM00000149'
    EXPORTING
      ...
      gt_farm_subtot_f   = gt_farm_subtot_f
      gt_farm_subtot_t   = gt_farm_subtot_t
      gv_langu           = gv_langu
      gv_datab           = lv_datab
      gv_datbi           = lv_datbi
      gv_rbtxt           = gv_rbtxt
...
**  CONCATENATE p_path '/' p_per+3(4) p_per(2) '_' p_region '.pdf' INTO
**                lv_file_path. " due to wave 3 changes commented

  CONCATENATE p_path '/' p_per+3(4) p_per(2) '_' s_region-low '.pdf' INTO
                lv_file_path. " due to wave 3 changes added
...
*--Validating Plant
**  IF p_werks IS NOT INITIAL. " due to Wave 3 changes commented
**    SELECT SINGLE iwerk
**      FROM t001w
**      INTO @DATA(lv_plant)
**     WHERE werks = @p_werks. " due to Wave 3 changes commented
...
*&---------------------------------------------------------------------*
*& Form TERRAINS_DATA_SFP_COPY
*&---------------------------------------------------------------------*
*& TERRAINS_DATA_SFP_COPY
*&---------------------------------------------------------------------*
FORM terrains_data_sfp_copy .
...
*--Workarae declaration
         ...
         lfp_outputparams TYPE sfpoutputparams,
         lfp_formoutput   TYPE fpformoutput.
  DATA: lt_terrain_dtls    TYPE tty_terrain_dtls,
        lt_farm_subtot     TYPE zabs_tty_farm_subtot,
        lt_farm_subtot_l   TYPE zabs_tty_farm_subtot,
...
*--To get FM name to be generated by Adobeform
  CALL FUNCTION 'FP_FUNCTION_MODULE_NAME'
    EXPORTING
      i_name     = zcl_abs_abap_maintain=>c_frmname_trndtls_copy "'ZABS_ADBFRM_TERRAIN_COPY'.
    IMPORTING
      e_funcname = lv_fm_name.
...
**    ls_farm_subtot-reason = ''.
**    ls_farm_subtot-rsdesc = ''.
**    ls_farm_subtot-qtd_plants = ''.
**    APPEND ls_farm_subtot TO lt_farm_subtot_l.
**    CLEAR ls_farm_subtot.
**  ENDLOOP.
**
**  SORT lt_farm_subtot_l BY region.
**  DELETE ADJACENT DUPLICATES FROM lt_farm_subtot_l.
