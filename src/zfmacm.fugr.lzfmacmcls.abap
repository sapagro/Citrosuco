...
*           on_view_changed_vlcl FOR EVENT view_changed
*                    OF /agri/cl_worklist_container
*        IMPORTING e_view,
...
*****Grid
      ...
      on_data_changed_vlcl FOR EVENT data_changed
                    OF /agri/cl_gui_alv_grid
        IMPORTING er_data_changed
                    e_onf4
                    e_onf4_before
      ...
      on_f4_request_vlcl FOR EVENT onf4
                    OF /agri/cl_gui_alv_grid
        IMPORTING e_fieldname e_fieldvalue
                    es_row_no er_event_data
                    et_bad_cells e_display,
      ...
      on_user_command_grid_vlcl FOR EVENT user_command
                    OF /agri/cl_gui_alv_grid
        IMPORTING e_ucomm,

      on_toolbar_grid_vlcl FOR EVENT toolbar
                    OF /agri/cl_gui_alv_grid
        IMPORTING e_object e_interactive sender.
...
****ON HOTSPOT CLICK
  ...
  METHOD on_data_changed_items.
    ...
    REFRESH: gt_items_modi.

    gs_variables-manual_changes = c_true.
    APPEND LINES OF er_data_changed->mt_mod_cells[] TO gt_items_modi .
    SORT gt_items_modi BY row_id.
    DELETE ADJACENT DUPLICATES FROM gt_items_modi COMPARING row_id.
  ...
  METHOD on_data_changed_vlcl.

    REFRESH: gt_items_modi_vlcl.

    gs_variables-manual_changes = c_true.
    APPEND LINES OF er_data_changed->mt_mod_cells[] TO gt_items_modi_vlcl .
    SORT gt_items_modi BY row_id.
    DELETE ADJACENT DUPLICATES FROM gt_items_modi_vlcl COMPARING row_id.
...
**    PERFORM attributes_navigate USING lwa_attr_value.
  ...
  METHOD on_f4_request_items.
    ...
    DATA: lwa_item_value  TYPE  zsc_fmacitm_fcat,
          lwa_item_value2 TYPE zsc_fmacitm_fcat.
...
*                   <lwa_acdata>         TYPE any.
    ...
    IF e_fieldname = 'TPLNR_FL'.
*...BOC-T_T.KONNO
*      PERFORM terrains_f4_values_get USING e_display
*                                  CHANGING lv_value
*                                           lv_changed
*                                           lv_subrc.
      PERFORM f4_terrains_values_get USING e_display
                                  CHANGING lv_value
                                           lv_changed
                                           lv_subrc.
*...EOC-T_T.KONNO
      ...
      WHEN ref_grid_items.
        IF gs_variables-document_mode NE c_mode_display.
          ...
          toolbar_button_insert e_object->mt_toolbar lwa_button space
                      ...
                      TEXT-006 space.
          toolbar_button_insert e_object->mt_toolbar lwa_button space
                     c_fcode-vol_calda_recalcular  icon_bw_formula_sap
                     TEXT-009 space.
        ENDIF.
    ENDCASE.
*
      ...
      WHEN ref_grid_vlcl.
        IF gs_variables-document_mode NE c_mode_display.
          toolbar_button_insert e_object->mt_toolbar lwa_button 3 space
                                space space space.
          toolbar_button_insert e_object->mt_toolbar lwa_button space
                      c_fcode-update_mass  icon_mass_change
                      TEXT-049 space.
        ENDIF.
    ENDCASE.
...
****Please select a row
  ...
  METHOD on_user_command_grid_vlcl.

    REFRESH gt_selected_rows_vldl.

    IF e_ucomm EQ c_fcode-update_mass.
      CALL METHOD ref_grid_vlcl->get_selected_rows
        IMPORTING
          et_index_rows = gt_selected_rows_vldl.
      IF NOT gt_selected_rows_vldl IS INITIAL.
****Desc Layout Row Delete
        CALL METHOD cl_gui_cfw=>set_new_ok_code
          EXPORTING
            new_code = c_fcode-update_mass.

      ELSEIF gt_selected_rows_vldl IS INITIAL.
****Please select a row
        MESSAGE ID '/AGRI/GLOBAL' TYPE 'I' NUMBER 321 INTO sy-msgli.
        message_simple space.
        e_ucomm = c_fcode-enter.
