...
*& text
*&---------------------------------------------------------------------*
*&      --> LWA_ITEM_LAYOUT_ACPOS
*&---------------------------------------------------------------------*
FORM new_position_set  CHANGING lv_acpos TYPE posnr.

  DATA: lwa_fmacitm TYPE zsc_fmacitm,
        lv_post     TYPE int4.

  LOOP AT gs_acdoc_infocus-x-acitm INTO lwa_fmacitm.
    IF lv_post LT lwa_fmacitm-acpos.
      lv_post = lwa_fmacitm-acpos.
    ENDIF.
  ENDLOOP.
  lv_acpos = lv_post + 1.
ENDFORM.
