FUNCTION ZFMAC_MEASUREMENT_GET.
*"--------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"     REFERENCE(ES_MDLMP) TYPE  /AGRI/S_FMACLMP
*"  EXCEPTIONS
...
**  ENDIF.
**
**  SELECT a~acnum a~tplnr_fl a~contr a~cmnum a~equnr
**         a~muser a~mdate a~mtime b~atinn b~atwrt
**         b~atflv "b~cunit
**            INTO CORRESPONDING FIELDS OF TABLE lt_acdata
**            FROM /agri/glachdr AS a INNER JOIN
**                 /agri/FMACatv AS b
**              ON b~acnum EQ a~acnum
