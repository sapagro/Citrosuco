...
FORM tabstrip_initialize .
  ...
  DATA: lv_domain             TYPE domname VALUE 'ZFMRCTSTAB',
...
*      BREAK-POINT.
*    ELSE.
*      DELETE lt_domain_values WHERE domvalue_l EQ 'SM'.
*    ENDIF.
...
****  Dose
****Prepare title
          WRITE icon_set_a AS ICON TO <lv_actual_tab>.
...
****General Tab
****Prepare title
          WRITE icon_bw_segment_sap AS ICON TO <lv_actual_tab>.
          CONCATENATE <lv_actual_tab> ls_domain_value-ddtext
                 INTO <lv_actual_tab>.
...
****Store other info for screen processing
          ...
          CONCATENATE c_prefix_tab_fcode lv_agtab
                 INTO ls_tabstrip_fcode-ts_fcode.
          ls_tabstrip_fcode-local_fcode = c_fcode-tab_version.
          APPEND ls_tabstrip_fcode TO gt_tabstrip_fcodes.

        WHEN 'BM'.
**** Bom Material List
****Prepare title
          WRITE icon_bom AS ICON TO <lv_actual_tab>.
          CONCATENATE <lv_actual_tab> ls_domain_value-ddtext
                 INTO <lv_actual_tab>.
...
****Store other info for screen processing
          ...
          CONCATENATE c_prefix_tab_fcode lv_agtab
                 INTO ls_tabstrip_fcode-ts_fcode.
          ls_tabstrip_fcode-local_fcode = c_fcode-tab_bom.
          APPEND ls_tabstrip_fcode TO gt_tabstrip_fcodes.
...
****Admin Data
        ...
        WHEN 'SM'.
****Produtos Similares
          WRITE icon_product_group AS ICON TO <lv_actual_tab>.
          CONCATENATE <lv_actual_tab> ls_domain_value-ddtext
                 INTO <lv_actual_tab>.
          ...
          CONCATENATE c_prefix_tab_fcode lv_agtab
                 INTO ls_tabstrip_fcode-ts_fcode.
          ls_tabstrip_fcode-local_fcode = c_fcode-tab_similar.
          APPEND ls_tabstrip_fcode TO gt_tabstrip_fcodes.
      ENDCASE.
    ENDLOOP.
  ENDIF.
...
*  ENDIF.
  ...
  CONCATENATE gs_rcdoc_infocus-x-rchdr-rcnum '' INTO lv_objval.
...
FORM texts_update .
 ...
 AND lv_texts_changed IS NOT INITIAL.
    ...
    IF gs_rcdoc_infocus-x-rchdr-updkz IS INITIAL.
