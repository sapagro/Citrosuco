*----------------------------------------------------------------------*
***INCLUDE LZGF_VX_GLCM_INTERFACEF01.
*----------------------------------------------------------------------*
...
* Program           : LZGF_VX_GLCM_INTERFACEF01                        *
...
*&---------------------------------------------------------------------*
*& Form global_data_intialize
*&---------------------------------------------------------------------*
FORM global_data_intialize.

*-- Clear global data
  CLEAR: gt_cskey, gt_cskey_upd, gv_tplnr, gv_bwtar, gv_datab, gv_iwerk,
         gs_glcmhdr.
...
*-- Validate whether the crop exist in Crop master process or not
  ##WARN_OK
  SELECT SINGLE cmnum
    FROM /agri/glcmprs
...
*-- A part from creation we need to check for other scenarios
*-- Validate Crop & variant with in validate date or not and get counter
  ...
  CASE cs_indata-indfield.
    WHEN gc_constants-insert.
      IF NOT gt_cskey_upd IS INITIAL AND cs_indata-crop_ex EQ space.
        READ TABLE gt_cskey_upd INTO DATA(ls_cskey_upd) INDEX 1.
        IF ( sy-subrc EQ 0 ) AND
           ( ls_cskey_upd-astat EQ gc_constants-astat_a ).
          PERFORM build_msgs USING gc_constants-msgty_error '161'
                                   space space
                          CHANGING ct_messages[].
        ENDIF.
      ENDIF.
...
*     AND astat    EQ gc_constants-astat_a
     AND iwerk    EQ gv_iwerk
     AND loevm    EQ space.
...
*-- Export the Memory variable(Valuation type) to BADI: WORKORDER_UPDATE
  EXPORT gv_bwtar TO MEMORY ID 'GV_BWTAR'.
  ...
  ls_order_comm-matnr = gs_glcmhdr-ymatnr.
...
*-- Local internal tables
  DATA: lt_cskey        TYPE /agri/t_glcs_key,
        ...
        lt_csprsor      TYPE ZVXTT_GLCSPRSOR, "/agri/t_glcsprsor,
...
"!Não EXISTE AQUI"
*    IF ps_indata-crop_ex EQ gc_constants-flag.
...
*-- Build messages
    ...
    IF sy-subrc EQ 0.
      ...
      ls_glcs_change-x-csprso  = lt_csprso.
      ls_glcs_change-x-csprst  = lt_csprst.
      "ls_glcs_change-x-csprsor = lt_csprsor.    "NÂO EXISTE AQUI"
      ls_glcs_change-x-csdfl   = lt_csdfl.
